...
async function buildXlsx(projectName, baseUrl, run, findings, outPath) {
  ...
  // --- One sheet per page ---
  // Excel sheet names can't contain * ? : \ / [ ], are capped at 31 chars
  // and must be unique case-insensitively. A page's title (used as the
  // sheet name) falls back to its URL on failed loads, which contains "/",
  // and many sitemap pages share a " | Site Name" title suffix.
  const sanitizeSheetName = (str) => (str || "").replace(/[*?:\\/[\]]/g, "-").replace(/^'+|'+$/g, "").trim() || "Page";
  const usedNames = new Set(["summary"]);
  for (const { pageUrl, pageTitle, rows } of pages) {
    const base = sanitizeSheetName(pageTitle || pageUrl);
