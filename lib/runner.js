/**
 * The actual QA engine. Given a base URL + extra paths, auto-discovers the
 * site's pages (sitemap.xml + main nav links, capped at DEFAULT_MAX_CRAWL_PAGES)
 * and drives real headless Chromium browser contexts, returning an array of
 * finding objects:
 *
 ...
 *
 * Checks, by category:
 *   Crawl                 auto-discovery crawl summary (pages found, cap reached, etc.)
 ...
 *   Links                 every internal link returns a non-error status; every
 *                         external link found gets a lightweight "loads OK" check
...
const { normalizeUrl, parseSitemapLocs, discoverNavLinks, buildCrawlQueue, checkExternalLink } = require("./crawler");
...
const DEFAULT_MAX_CRAWL_PAGES = 25;
...
async function checkInternalLinks(context, page, url, title, targetHost, findings) {
  const { internal, external } = await page.evaluate((host) => {
    const internal = [];
    const external = [];
    for (const a of document.querySelectorAll("a[href]")) {
      ...
      try {
        ...
        (u.hostname === host ? internal : external).push(href);
      } catch {
  ...
  const uniqueLinks = uniqueInternal.slice(0, MAX_INTERNAL_LINKS_PER_PAGE);
  const broken = [];
  ...
  const label = `Internal links return a non-error status (checked ${uniqueLinks.length}${uniqueInternal.length > uniqueLinks.length ? ` of ${uniqueInternal.length}` : ""})`;
  const shown = broken.slice(0, 8);
  const f = finding(
  ...
  return { internalLinks: uniqueInternal, externalLinks: [...new Set(external)] };
}
...
async function checkPage(context, url, findings, isHomepage = false, crawlState = null) {
  const page = await context.newPage();
  const consoleErrors = [];
  ...
  try {
    ...
    const { internalLinks, externalLinks } = await checkInternalLinks(context, page, url, title, targetHost, findings);

    if (crawlState) {
      for (const rawLink of internalLinks) {
        const normalized = normalizeUrl(rawLink, url);
        if (!normalized || crawlState.visited.has(normalized) || crawlState.queued.has(normalized)) continue;
        if (crawlState.queued.size >= crawlState.maxPages) continue;
        crawlState.queued.add(normalized);
      ...
      for (const rawLink of externalLinks) {
        ...
        const alreadyChecked = crawlState.externalLinkRegistry.has(normalized);
        const result = await checkExternalLink(context, normalized, crawlState.externalLinkRegistry, LINK_CHECK_TIMEOUT);
        if (!alreadyChecked) {
          findings.push(
            finding(url, title, "Links", "External link returns a non-error status",
              result.ok ? "PASS" : "FAIL",
              result.detail ? `${result.status} ${normalized} (${result.detail})` : `${result.status} ${normalized}`)
          );
        }
      }
    }
...
async function runQa(baseUrl, extraPaths, screenshotDir, options = {}) {
  const maxPages = options.maxPages || DEFAULT_MAX_CRAWL_PAGES;
  const findings = [];
  ...
  try {
    ...
    const normalizedBase = normalizeUrl(baseUrl, baseUrl) || baseUrl;

    const extraPathUrls = extraPaths
      .map((p) => p.trim())
      .filter(Boolean)
      .map((p) => normalizeUrl(new URL(p, baseUrl).toString(), baseUrl))
      .filter(Boolean);

    // Load the homepage once, purely to read its main nav/header links.
    let navUrls = [];
    const homePage = await context.newPage();
    try {
      await homePage.goto(baseUrl, { timeout: 20000, waitUntil: "load" });
      navUrls = (await discoverNavLinks(homePage, targetHost))
        .map((u) => normalizeUrl(u, baseUrl))
        .filter(Boolean);
    } catch {
      /* homepage failed to load — checkPage will report the real failure below */
    } finally {
      await homePage.close();
    }

    const sitemapUrls = await parseSitemapLocs(context, baseUrl);

    const { queue: seedQueue, mandatoryCount, sitemapCount, sitemapExcluded } = buildCrawlQueue({
    ...
    const crawlState = {
      visited: new Set(),
      queued: new Set(seedQueue),
      queue: [...seedQueue],
      maxPages,
      externalLinkRegistry: new Map(),
    };

    while (crawlState.queue.length) {
      const url = crawlState.queue.shift();
      if (crawlState.visited.has(url)) continue;
      crawlState.visited.add(url);
      await checkPage(context, url, findings, url === normalizedBase, crawlState);
    }
    await context.close();

    const finalUrls = [...crawlState.visited];
    const discoveredCount = Math.max(finalUrls.length - mandatoryCount - sitemapCount, 0);

    findings.push(
      finding(
        normalizedBase, "Crawl Summary", "Crawl", "Auto-discovery crawl summary",
        sitemapExcluded > 0 ? "WARNING" : "PASS",
        `${finalUrls.length} page(s) crawled (cap ${maxPages}): ${mandatoryCount} homepage/nav/extra-path, ` +
          `${sitemapCount} from sitemap, ${discoveredCount} discovered via in-page links.\n` +
          (sitemapExcluded > 0 ? `${sitemapExcluded} sitemap URL(s) excluded due to the ${maxPages}-page cap.\n` : "") +
          `${crawlState.externalLinkRegistry.size} unique external link(s) checked.`
      )
    );

    // Responsive pass: re-check every crawled page at mobile and tablet sizes.
